// interop_verify_go.go — Comprehensive Go cross-language PQC verifier.
//
...
//
// Usage (must be run from the go/ directory):
//   cd PQC-Standards-Implementation/go
//   go run ../interop/interop_verify_go.go [VECTORS_DIR]
//
// VECTORS_DIR defaults to ../interop/vectors relative to the go/ working dir.
...
func verifyMLKEM(alg string, data []byte) (pass bool, errMsg string) {
	var v mlkemVector
...
func verifyMLDSA(alg string, data []byte) (pass bool, errMsg string) {
	var v mldsaVector
	if err := json.Unmarshal(data, &v); err != nil {
		return false, fmt.Sprintf("JSON parse error: %v", err)
	}
...
func verifySLHDSA(alg string, data []byte) (pass bool, errMsg string) {
	var v slhdsaVector
	if err := json.Unmarshal(data, &v); err != nil {
		return false, fmt.Sprintf("JSON parse error: %v", err)
	}
...
func main() {
	...
	for _, entry := range entries {
		...
		// Extract algorithm name from JSON
		var header struct {
			Algorithm string `json:"algorithm"`
		}
